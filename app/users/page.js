...
export default async function UsersPage() {
  ...
  if (!["admin", "atendimento"].includes(profile?.role)) redirect("/portal");

  const [{ data: systemUsers }, { data: clients }, { data: approvers }] =
    await Promise.all([
      supabase
        .from("users_with_clients")
        .select("*")
        .order("created_at", { ascending: false }),
      getClientsWithApproverCount(supabase, session.user.id).then((r) => ({
        data: r.data
      ...
      (async () => {
        ...
        // Fallback
        const { data: approverRows2 } = await supabase
          .from("approvers")
          .select("*")
          .order("name");
        if (!approverRows2) return { data: [], error: null };
        const cids = [
          ...new Set(approverRows2.map((a) => a.client_id).filter(Boolean)),
        ];
        let cmap = {};
        if (cids.length) {
        ...
        return {
          data: approverRows2.map((a) => ({
            ...a,
            clients: cmap[a.client_id] || null,
          })),
          error: null,
        };
      })(),
