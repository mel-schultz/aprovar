...
export default function UsuariosManagementClient() {
  ...
  const handleDeleteUsuario = async (id, email) => {
    if (email === 'mel.schultz@yahoo.com') {
      toast.error('Não é possível deletar o Super Admin!')
      return
    }
                ...
                <tr key={usuario.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <td style={{ padding: '16px' }}>
                    <span style={{
                      ...
                      padding: '4px 12px',
                      borderRadius: '9999px',
                      backgroundColor: ROLES[usuario.role]?.bg || '#f3f4f6',
                      color: ROLES[usuario.role]?.color || '#666',
                      fontSize: '12px',
                      fontWeight: '600'
                    }}>
