...
const PLANS = [
  {
    id: 'starter', name: 'Iniciante', description: 'Para freelancers e profissionais solo.',
  ...
  {
    id: 'intermediate', name: 'Intermediário', description: 'Para agências com equipe.', highlight: true,
  ...
  {
    id: 'complete', name: 'Completo', description: 'Para agências que precisam de escala.',
...
export default function BillingClient({ profile }) {
  ...
  async function handleSelect(planId) {
    ...
    toast.success(`Plano ${planId} ativado! (integre o Stripe em produção)`)
    setLoading(null)
  }
  ...
  return (
    <div className="page-enter">
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 36 }}>
        {CYCLES.map(c => (
          <button key={c.id} onClick={() => setCycle(c.id)} style={{ padding: '8px 18px', borderRadius: 99, fontSize: 14, fontWeight: 500, cursor: 'pointer', background: cycle === c.id ? 'var(--brand)' : 'var(--surface)', color: cycle === c.id ? '#fff' : 'var(--text-2)', border: `1.5px solid ${cycle === c.id ? 'var(--brand)' : 'var(--border)'}`, transition: 'all .15s', display: 'flex', alignItems: 'center', gap: 6 }}>
            {c.label}
            {c.discount && <span style={{ fontSize: 11, background: cycle === c.id ? 'rgba(255,255,255,.25)' : 'var(--brand-light)', color: cycle === c.id ? '#fff' : 'var(--brand)', borderRadius: 99, padding: '1px 6px' }}>{c.discount}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20, maxWidth: 1000, margin: '0 auto' }}>
        {PLANS.map(plan => {
          ...
          return (
            <div key={plan.id} style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg)', border: plan.highlight ? '2px solid var(--brand)' : '1px solid var(--border)', boxShadow: plan.highlight ? 'var(--shadow-lg)' : 'var(--shadow-sm)', padding: 28, position: 'relative', transform: plan.highlight ? 'scale(1.03)' : 'scale(1)' }}>
              {plan.highlight && (
              ...
              <div style={{ marginBottom: 24 }}>
                <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 38 }}>R$ {price.toFixed(2).replace('.', ',')}</span>
                <span style={{ fontSize: 13, color: 'var(--text-2)' }}>/mês</span>
              </div>
              <Button onClick={() => handleSelect(plan.id)} loading={loading === plan.id} disabled={isCurrent} variant={plan.highlight ? 'primary' : 'outline'} style={{ width: '100%', padding: 12, marginBottom: 24 }}>
                {isCurrent ? 'Plano atual' : `Assinar ${plan.name}`}
              </Button>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {plan.features.map(f => (
                  <div key={f.label} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    {f.ok ? <Check size={16} color="var(--brand)" style={{ flexShrink: 0, marginTop: 1 }} /> : <X size={16} color="var(--text-3)" style={{ flexShrink: 0, marginTop: 1 }} />}
