...
export default function TeamClient({ initialMembers, userId }) {
  ...
  async function toggleRole(member) {
    ...
    const { data, error } = await supabase
      .from('team_members').update({ role: newRole }).eq('id', member.id).select().single()
    if (error) { toast.error(error.message); return }
    setMembers(m => m.map(mb => mb.id === data.id ? data : mb))
    toast.success(`Função alterada para ${newRole === 'admin' ? 'Admin' : 'Membro'}.`)
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-2)', fontSize: 14 }}>
            Adicione membros para colaborar nos projetos. Membros ilimitados.
          </p>
        </div>
      ...
      {members.length === 0 ? (
        <Card>
          <EmptyState
            ...
            description="Convide colaboradores para gerenciar projetos juntos."
            action={<Button onClick={() => setModal(true)}><Plus size={16} /> Convidar</Button>}
          />
        </Card>
      ) : (
        <Card>
          {members.map((m, i) => (
            <div
              key={m.id}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: i < members.length - 1 ? '1px solid var(--border)' : 'none' }}
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <button
                  ...
                  style={{ fontSize: 12, fontWeight: 500, background: m.role === 'admin' ? 'var(--brand-light)' : 'var(--surface-3)', color: m.role === 'admin' ? 'var(--brand)' : 'var(--text-2)', borderRadius: 6, padding: '4px 12px', border: 'none', cursor: 'pointer' }}
                >
