    ...
    <form onSubmit={handleSubmit}>
      ...
      <FormField label="Título do conteúdo *">
        <input
          ...
          placeholder="Post Instagram — Julho"
          required
        />
      </FormField>
      <FormField label="Descrição / orientações">
        <textarea
          ...
          rows={3}
          placeholder="Contexto sobre este conteúdo..."
        />
      </FormField>
...
function DeliverableDetailsModal({ deliverable, clients, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    scheduled_at: deliverable.scheduled_at
      ? new Date(deliverable.scheduled_at).toISOString().slice(0, 16)
      : "",
  });
  const [file, setFile] = useState(null);
  const [loading, setLoading] = useState(false);
  ...
  const clientName =
    clients.find((c) => c.id === deliverable.client_id)?.name ||
      ...
      {!isEditing ? (
        <div>
              ...
              {deliverable.network ? (
                <span
                  style={{
                    fontSize: 11,
                    background:
                      (NETWORK_COLORS[deliverable.network] || "var(--brand)") +
                      "22",
                    color:
                      NETWORK_COLORS[deliverable.network] || "var(--brand)",
                    borderRadius: 4,
          ...
          {deliverable.scheduled_at && (
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 14, color: "var(--text)", margin: 0 }}>
                {new Date(deliverable.scheduled_at).toLocaleDateString(
                  "pt-BR",
                  {
                    year: "numeric",
                    month: "long",
        ...
        <form onSubmit={handleSave}>
          ...
          <FormField label="Substituir arquivo (opcional)">
            ...
            >
              Se não fazer upload, manterá o arquivo atual
            </p>
            <input
              type="file"
              accept="image/*,video/*,.pdf"
          ...
          >
            ...
            <Button type="submit" loading={loading}>
              <Upload size={14} /> Salvar alterações
            </Button>
          </div>
        </form>
      )}
    ...
    <div className="page-enter">
      ...
      {filtered.length === 0 ? (
        <Card>
          <EmptyState
            icon={CheckSquare}
            title="Nenhum entregável"
            description="Crie um entregável e envie o link para seu cliente aprovar."
            action={
              <Button onClick={() => setModal(true)}>
                <Plus size={16} /> Criar entregável
              </Button>
            }
          />
        </Card>
              ...
              >
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-2)" }}>
                    ...
                    {d.scheduled_at &&
                      ` · Agendado: ${new Date(d.scheduled_at).toLocaleDateString("pt-BR")}`}
                  </p>
                </div>
                <div
                ...
                >
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => window.open(`/approve/${d.token}`, "_blank")}
                  >
                    <Eye size={13} /> Ver
                  </Button>
                </div>
