...
const BRAND_COLORS = ['#0ea472','#6366f1','#f59e0b','#ef4444','#8b5cf6','#ec4899','#14b8a6','#f97316','#3b82f6','#10b981','#e11d48','#0ea5e9']

export default function SettingsClient({ profile, userEmail }) {
  const supabase = createClient()
  const [form, setForm] = useState({ full_name: profile?.full_name || '', company: profile?.company || '', brand_color: profile?.brand_color || '#0ea472', logo_url: profile?.logo_url || '' })
  ...
  async function handleSave(e) {
    ...
    const { error } = await supabase.from('profiles').update({ ...form, logo_url }).eq('id', profile.id)
    ...
    <div className="page-enter" style={{ maxWidth: 680 }}>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        <form onSubmit={handleSave}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <FormField label="Seu nome"><input value={form.full_name} onChange={e => set('full_name', e.target.value)} placeholder="João Silva" /></FormField>
            <FormField label="Empresa / Agência"><input value={form.company} onChange={e => set('company', e.target.value)} placeholder="Minha Agência" /></FormField>
          </div>
          <p style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 16 }}>E-mail: <strong>{userEmail}</strong></p>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ background: 'var(--surface-3)', borderRadius: 10, padding: 16, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          {form.logo_url
            ...
            : <div style={{ width: 40, height: 40, borderRadius: 10, background: form.brand_color, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18 }}>{(form.company || 'A').charAt(0)}</div>
          }
          <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, color: form.brand_color }}>{form.company || 'Sua Agência'}</span>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
            {BRAND_COLORS.map(c => (
              <button key={c} onClick={() => set('brand_color', c)} style={{ width: 32, height: 32, borderRadius: 8, background: c, border: 'none', cursor: 'pointer', outline: form.brand_color === c ? `3px solid ${c}` : '3px solid transparent', outlineOffset: 2, transition: 'outline .12s' }} />
            ))}
      ...
      <Card style={{ padding: 24 }}>
        ...
        <form onSubmit={handlePasswordChange}>
          <FormField label="Nova senha"><input type="password" value={pwForm.next} onChange={e => setPwForm(f => ({ ...f, next: e.target.value }))} placeholder="••••••••" minLength={6} required /></FormField>
          <FormField label="Confirmar nova senha"><input type="password" value={pwForm.confirm} onChange={e => setPwForm(f => ({ ...f, confirm: e.target.value }))} placeholder="••••••••" minLength={6} required /></FormField>
