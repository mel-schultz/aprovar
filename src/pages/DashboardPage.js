    ...
    <Card style={{ padding: 24 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ fontSize: 32, fontFamily: 'var(--font-display)', fontWeight: 800, color }}>{value}</p>
...
export default function DashboardPage() {
  ...
  const [stats, setStats] = useState({ clients: 0, pending: 0, approved: 0, scheduled: 0 })
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setStats({ clients: clients || 0, pending: pending || 0, approved: approved || 0, scheduled: scheduled || 0 })
  ...
  const trialEnd = profile?.trial_ends ? new Date(profile.trial_ends) : null
    ...
    <div className="page-enter">
      ...
      {isTrial && !trialExpired && (
        <div style={{ background: 'var(--accent)', borderRadius: 'var(--radius)', padding: '12px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 14, color: '#7c2d12', fontWeight: 500 }}>
            ...
            <Link to="/billing" style={{ fontWeight: 700, textDecoration: 'underline' }}>Assine um plano</Link>
      ...
      <Card>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Link to="/approvals" style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'var(--brand)', fontSize: 13, fontWeight: 500 }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: 40, textAlign: 'center' }}>
            ...
            <Link to="/approvals" style={{ color: 'var(--brand)', fontWeight: 600, fontSize: 14, display: 'inline-block', marginTop: 8 }}>
          ...
          <div>
            {recent.map((d, i) => (
              <div key={d.id} style={{
                ...
                padding: '14px 24px', borderBottom: i < recent.length - 1 ? '1px solid var(--border)' : 'none',
              }}>
                <div>
