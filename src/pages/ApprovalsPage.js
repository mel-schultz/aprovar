...
function ApprovalLinkCard({ deliverable }) {
  const url = `${window.location.origin}/approve/${deliverable.token}`
...
export default function ApprovalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: dels }, { data: cls }] = await Promise.all([
      ...
      supabase.from('clients').select('id,name').eq('profile_id', user.id),
    ])
  ...
  async function handleSave(form) {
    const { data, error } = await supabase.from('deliverables').insert({ ...form, profile_id: user.id, status: 'pending' }).select('*, clients(name)').single()
  ...
  return (
    <div className="page-enter">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26 }}>Aprovações</h1>
          <p style={{ color: 'var(--text-2)', fontSize: 14, marginTop: 4 }}>Envie e gerencie conteúdos para aprovação.</p>
        </div>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState icon={CheckSquare} title="Nenhum entregável" description="Crie um entregável e envie o link para seu cliente aprovar." action={<Button onClick={() => setModal(true)}><Plus size={16} /> Criar entregável</Button>} />
        </Card>
      ) : (
