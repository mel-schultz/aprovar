  ...
  return (
    <form onSubmit={handleSubmit}>
      <FormField label="Nome do cliente *">
        <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="Empresa XYZ" required />
      </FormField>
      <FormField label="E-mail do cliente">
        <input type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="contato@empresa.com" />
      </FormField>
      <FormField label="WhatsApp">
        <input value={form.whatsapp} onChange={e => set('whatsapp', e.target.value)} placeholder="+55 11 9..." />
      </FormField>
      <FormField label="Observações">
        <textarea value={form.notes} onChange={e => set('notes', e.target.value)} rows={3} placeholder="Contexto sobre o cliente..." />
      </FormField>
...
export default function ClientsPage() {
  ...
  const [modal, setModal] = useState(null) // null | 'create' | {edit: client} | {approvers: client}
  ...
  async function handleSave(form) {
    if (modal === 'create') {
      const { data, error } = await supabase.from('clients').insert({ ...form, profile_id: user.id }).select('*, approvers(count)').single()
      ...
      if (error) { toast.error(error.message); return }
      setClients(c => c.map(cl => cl.id === data.id ? data : cl))
    ...
    <div className="page-enter">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-2)', fontSize: 14, marginTop: 4 }}>Gerencie seus clientes e aprovadores.</p>
        </div>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState icon={Users} title="Nenhum cliente ainda" description="Adicione seu primeiro cliente para começar a enviar conteúdo para aprovação." action={<Button onClick={() => setModal('create')}><Plus size={16} /> Criar cliente</Button>} />
        </Card>
      ) : (
            ...
            <Card key={client.id} style={{ padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 10, background: 'var(--brand-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 18, color: 'var(--brand)' }}>
              ...
              <button onClick={() => setModal({ approvers: client })} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', background: 'var(--surface-3)', border: 'none', borderRadius: 8, padding: '10px 14px', cursor: 'pointer' }}>
                <span style={{ fontSize: 13, color: 'var(--text-2)', fontWeight: 500 }}>
                  Aprovadores: {client.approvers?.[0]?.count || 0}
                </span>
