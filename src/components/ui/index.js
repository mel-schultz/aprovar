...
export function Button({ children, variant = 'primary', size = 'md', loading, className = '', ...props }) {
  ...
  return (
    <button
      style={{ ...base, ...variants[variant], ...sizes[size], opacity: loading ? .6 : 1 }}
      disabled={loading || props.disabled}
      className={className}
      {...props}
    >
      {loading ? <span style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,.4)', borderTopColor: '#fff', borderRadius: '50%', display: 'inline-block', animation: 'spin .6s linear infinite' }} /> : null}
      {children}
    </button>
  )
}
...
export function Modal({ open, onClose, title, children, width = 520 }) {
  ...
  return (
    ...
    >
      <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)', width: '100%', maxWidth: width, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
...
export function EmptyState({ icon: Icon, title, description, action }) {
  return (
    <div style={{ textAlign: 'center', padding: '60px 24px' }}>
      {Icon && <div style={{ margin: '0 auto 16px', width: 56, height: 56, background: 'var(--brand-light)', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
/* ── spin keyframe (global injection) ───────────── */
